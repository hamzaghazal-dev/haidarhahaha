...
class TripaneerScraper:
    ...
    def login(self):
        ...
        login_data = {
            'username': 'surfgazmmorocco@gmail.com',
            'password': 'Haidar2024',
        }
    ...
    def calculate_nights(self, arrival_date, departure_date):
    ...
    def determine_hostel(self, package_name):
    ...
    def extract_guest_and_room_info(self, conversation_link):
    ...
    def extract_bookings_from_list(self, booking_list, booking_type="current"):
        """Extract bookings from a specific list (current or upcoming)"""
        if not booking_list:
            return []
            
        booking_items = booking_list.find_all('li')
        print(f"Found {len(booking_items)} bookings in {booking_type} section")
        ...
        for index, item in enumerate(booking_items):
            try:
                ...
                print(f"Extracted {booking_type} booking {index + 1}: {full_name} - {guests} guests - {room_type}")
                
            except Exception as e:
                print(f"Error extracting {booking_type} booking {index + 1}: {e}")
                continue
    ...
    def extract_booking_data(self):
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Failed to fetch bookings page: {response.status_code}")
                return []
            ...
            print(f"Found {len(booking_lists)} booking lists on the page")
            ...
            if len(booking_lists) > 0:
                ...
                print("No current bookings list found")
            ...
            if len(booking_lists) > 1:
                ...
                print("No upcoming bookings list found")
            
            return all_bookings
            
        except Exception as e:
            print(f"Error fetching bookings page: {e}")
            return []

    def run(self):
        """Main function to run the scraper"""
        print("Starting Tripaneer scraper...")
        ...
        bookings_data = self.extract_booking_data()
        
        # Save to JSON file
        if bookings_data:
            output_file = "all_bookings.json"
            ...
            # Print summary
            print("\n=== EXTRACTION SUMMARY ===")
            current_count = sum(1 for b in bookings_data if b['booking_type'] == 'current')
            upcoming_count = sum(1 for b in bookings_data if b['booking_type'] == 'upcoming')
            
            print(f"Current bookings: {current_count}")
            print(f"Upcoming bookings: {upcoming_count}")
            print()
            
            for i, booking in enumerate(bookings_data, 1):
                booking_type = booking['booking_type'].capitalize()
                print(f"{i}. [{booking_type}] {booking['full_name']} - {booking['hostel']}")
                print(f"   Arrival: {booking['arrival_date']} - Departure: {booking['departure_date']}")
